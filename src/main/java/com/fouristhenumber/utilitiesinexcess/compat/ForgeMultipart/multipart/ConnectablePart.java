...
public abstract class ConnectablePart extends MaterialBasedPart {
    ...
    static {
        ...
        map.put(
            ForgeDirection.DOWN,
            new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.WEST, ForgeDirection.SOUTH,
                ForgeDirection.EAST });
        map.put(
            ForgeDirection.UP,
            new ForgeDirection[] { ForgeDirection.SOUTH, ForgeDirection.WEST, ForgeDirection.NORTH,
                ForgeDirection.EAST });
        map.put(
            ForgeDirection.EAST,
            new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.DOWN, ForgeDirection.SOUTH,
                ForgeDirection.UP });
        map.put(
            ForgeDirection.WEST,
            new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.UP, ForgeDirection.SOUTH,
                ForgeDirection.DOWN });
        map.put(
            ForgeDirection.NORTH,
            new ForgeDirection[] { ForgeDirection.UP, ForgeDirection.WEST, ForgeDirection.DOWN, ForgeDirection.EAST });
        map.put(
            ForgeDirection.SOUTH,
            new ForgeDirection[] { ForgeDirection.DOWN, ForgeDirection.WEST, ForgeDirection.UP, ForgeDirection.EAST });
    ...
    public boolean canConnectOnSide(ForgeDirection side) {
        ...
        if (block.isSideSolid(world(), offsetX, offsetY, offsetZ, side.getOpposite()) && block.renderAsNormalBlock()
            && block.getMaterial()
                .isOpaque()) {
            return true;
            ...
            if (world().getTileEntity(offsetX, offsetY, offsetZ) instanceof TileMultipart mpTile) {
                for (TMultiPart part : mpTile.jPartList()) {
                    if (part instanceof ConnectablePart cPart && part.getClass() == this.getClass()
        ...
        switch (downDirection) {
            ...
            case UNKNOWN -> {
                throw new IllegalArgumentException(
                    "Switch falloff in transforming the cullmask in a connectable part.");
            }
        }
        return mask;
    }
    ...
    public boolean drawConnecableHighLight(MovingObjectPosition hit, EntityPlayer player, float frame,
        ...
        for (Pair<Integer, Cuboid6> cubeWithDirInfo : highlightCuboidList) {
            AxisAlignedBB bb = cubeWithDirInfo.second()
                .toAABB()
                .offset(x(), y(), z());
            ...
            drawOutlinedBoundingBox(
                bb.expand(0.002F, 0.002F, 0.002F)
