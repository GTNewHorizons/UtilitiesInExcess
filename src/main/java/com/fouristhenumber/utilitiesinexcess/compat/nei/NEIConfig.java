...
public class NEIConfig implements IConfigureNEI {
    ...
    // spotless:off
    public static final ArrayList<ItemStack> ALIASED_ITEMSTACKS = new ArrayList<>(List.of(
    ...
    public static void initAliasItems() {
        for (int i = 0; i < 16; i++) {
            ALIASED_ITEMSTACKS.add(new ItemStack(ModBlocks.LAPIS_AETHERIUS.getItem(), 1, i));
        }

        for (int i = 0; i < 10; i++) {
            ALIASED_ITEMSTACKS.add(new ItemStack(ModBlocks.DECORATIVE_GLASS.getItem(), 1, i));
        }
    }

    public static void registerAliases() {
        for (ItemStack itemStack : ALIASED_ITEMSTACKS) {
