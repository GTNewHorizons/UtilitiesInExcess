...
public class PacketUnderworldAttack implements IMessage {
    ...
    public static class Handler implements IMessageHandler<PacketUnderworldAttack, IMessage> {
        ...
        public IMessage onMessage(PacketUnderworldAttack message, MessageContext ctx) {
            ...
            Vector3d a = new Vector3d(player.posX + Math.cos(theta1) * 2, player.posY, player.posZ + Math.sin(theta1) * 2);
            Vector3d b = new Vector3d(player.posX + Math.cos(theta2) * 2, player.posY, player.posZ + Math.sin(theta2) * 2);

            for (float k = 0; k <= 1f; k += 0.05f) {
                Vector3d v = new Vector3d(b).sub(a).mul(k).add(a);

                world.spawnParticle("crit",
                    v.x, v.y, v.z,
                    rng.nextDouble() * 0.02 - 0.01, rng.nextDouble() * 0.02 - 0.01, rng.nextDouble() * 0.02 - 0.01);
            }

            Vector3d v = new Vector3d(b).sub(a).mul(rng.nextDouble()).add(a);
